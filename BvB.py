...
class Connect:
    ...
    def __init__(self, board_size_x=5, board_size_y=4):
        ...
        self.player_turn = PLAYERS[PLAYER_AI2]
    ...
    def print_turn(self):
        if self.player_turn == PLAYERS[PLAYER_AI1]:
            print('It is AI1 to play')
        else:
            print('It is AI2 to play')

    # Determine if the game has a winner between the human and AI
    def has_winner(self):
        if self.has_a_row(PLAYER_AI1, WINNING_SEQUENCE_COUNT):
            return "AI1 won"
        elif self.has_a_row(PLAYER_AI2, WINNING_SEQUENCE_COUNT):
            return "AI2 won"
        return 0

    # Get the score for the AI
    def get_score_for_ai(self):
        if self.has_a_row(PLAYER_AI1, 4):
            return -10
        if self.has_a_row(PLAYER_AI2, 4):
            return 10
        return 0
    ...
    def play_move(self, slot):
        if 0 <= slot <= 4:
            if not self.is_slot_full(slot):
                if self.player_turn == PLAYERS[PLAYER_AI2]:
                    self.execute_move(PLAYER_AI2, slot)
                else:
                    self.execute_move(PLAYER_AI1, slot)
                self.player_turn *= -1
                return True
            return False
        return False
